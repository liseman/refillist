...
set :scm,             :git
set :repository,      "https://github.com/liseman/refillist.git"
...
role :web,    "54.241.244.80"
role :app,    "54.241.244.80"
role :db,     "54.241.244.80", :primary => true
...
default_environment["GEM_HOME"]     = "/usr/local/rvm/gems/ruby-1.9.3-p194@refillist"
default_environment["GEM_PATH"]     = "/usr/local/rvm/gems/ruby-1.9.3-p194@refillist:/usr/local/rvm/gems/ruby-1.9.3-p194@global"
...
namespace :deploy do
  ...
  task :restart, :except => { :no_release => true } do
    run "kill -s USR2 `cat /tmp/unicorn.refillist.pid`"
  end
  ...
  task :stop, :except => { :no_release => true } do
    run "kill -s QUIT `cat /tmp/unicorn.refillist.pid`"
  end  
